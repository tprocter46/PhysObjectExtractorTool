...
double
TruthJetAnalyzer::getBtagEfficiency(double pt){
...
double
TruthJetAnalyzer::getCtagEfficiency(double pt){
...
double
TruthJetAnalyzer::getLFtagEfficiency(double pt){
...
double
TruthJetAnalyzer::getBorCtagSF(double pt, double eta){
  if (pt > 670.) pt = 670;
...
double
TruthJetAnalyzer::getLFtagSF(double pt, double eta){
  if (pt > 1000.) pt = 1000;
  if(fabs(eta) > 2.4 or pt<20.) return 1.0;
  if(eta < 0.8)
...
double
TruthJetAnalyzer::uncertaintyForBTagSF( double pt, double eta){
...
double
TruthJetAnalyzer::uncertaintyForLFTagSF( double pt, double eta){
  if (pt > 1000.)  pt = 1000;
  if(fabs(eta) > 2.4 or pt<20.)  return 0;
  if(eta < 0.8)
...
std::vector<float>
TruthJetAnalyzer::factorLookup(float eta) { //used in jet loop for JER factor value
  //eta input is > 0
